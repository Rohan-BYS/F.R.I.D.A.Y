...
class NexusMCPServer:
    ...
    async def _call_tool(self, name: str, args: Dict) -> Any:
        if name == "browse":
            ...
            return await self.router.snapshot(mode=mode)
        elif name == "click":
            return await self.router.click(args["ref"])
        elif name == "fill":
            return await self.router.fill(args["ref"], args["text"])
        elif name == "screenshot":
            return await self.router.screenshot(args.get("url"))
        elif name == "act":
            return {"status": "success", "message": f"Action '{args['action']}' simulated"}
        elif name == "extract":
            return {"status": "success", "message": f"Extraction '{args['prompt']}' simulated"}
        elif name == "observe":
            tree = await self.router.snapshot(mode="pruned")
            shot = await self.router.tier2.capture_screenshot()
