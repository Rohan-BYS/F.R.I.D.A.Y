...
TOOLS = [
    ...
    {
        "name": "snapshot",
        "description": "Get current page AXTree with @eN refs or pruned interactive tree",
        "inputSchema": {
            "type": "object",
            "properties": {
                "mode": {"type": "string", "enum": ["axtree", "pruned"], "description": "Type of snapshot to get"}
            },
            "required": []
        }
    },
    {
        "name": "click",
        "description": "Click element by @eN reference handle or pruned index",
        "inputSchema": {
            "type": "object",
    ...
    {
        "name": "fill",
        "description": "Type text into form input by @eN reference handle or pruned index",
        "inputSchema": {
            "type": "object",
            "properties": {
                "ref": {"type": "string"},
    ...
    {
        "name": "act",
        "description": "Natural language action primitive (e.g. 'click sign in', 'type email@test.com into email')",
        "inputSchema": {
            "type": "object",
            "properties": {
                "action": {"type": "string", "description": "The action to perform in natural language"}
            },
            "required": ["action"]
        }
    },
    {
        "name": "extract",
        "description": "Natural language extraction primitive",
        "inputSchema": {
            "type": "object",
            "properties": {
                "prompt": {"type": "string"}
    ...
    {
        "name": "observe",
        "description": "High level observation primitive, returns pruned DOM and screenshot",
        "inputSchema": {
            "type": "object",
            "properties": {},
            "required": []
...
async def dispatch_tool(name: str, args: Dict[str, Any]) -> Any:
    ...
    if name == "browse":
        ...
        mode = args.get("mode", "axtree")
        return await router.snapshot(mode=mode)
    elif name == "click":
        return await router.click(args["ref"])
    elif name == "fill":
        return await router.fill(args["ref"], args["text"])
    elif name == "screenshot":
        return await router.screenshot(args.get("url"))
    elif name == "act":
        return {"status": "success", "message": f"Action '{args['action']}' simulated (Semantic LLM mapping deferred to Agent)"}
    elif name == "extract":
        return {"status": "success", "message": f"Extraction for '{args['prompt']}' simulated (Deferred to Agent/Tier3)"}
    elif name == "observe":
        tree = await router.snapshot(mode="pruned")
        shot = await router.tier2.capture_screenshot()
        return {"tree": tree, "screenshot_base64": shot}
