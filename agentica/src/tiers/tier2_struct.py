...
class Tier2StructEngine:
    ...
    PRUNING_JS = """
    function getInteractiveTree() {
      const elements = document.querySelectorAll('a, button, input, select, textarea, [role="button"], [onclick], [tabindex="0"]');
      const cleanList = [];
      let index = 1;
      
      // Remove old markers
      document.querySelectorAll('.friday-marker').forEach(m => m.remove());
      ...
      elements.forEach((el) => {
        ...
        if (rect.width > 0 && rect.height > 0 && window.getComputedStyle(el).visibility !== 'hidden') {
          ...
          cleanList.push({
            ...
            type: el.type || '',
            text: (el.innerText || el.placeholder || el.ariaLabel || '').trim().slice(0, 50),
    ...
    def __init__(self):
        ...
        self._pruned_elements: List[Dict] = []
    ...
    async def _ensure_page(self, headless: bool = True):
        ...
        if self.page is None:
            ...
            await self.page.add_init_script(self.STEALTH_EVASION_JS)
    ...
    async def dismiss_interruptions(self):
    ...
    async def get_accessibility_tree(self, url: str, headless: bool = True) -> str:
        ...
        try:
            ...
            await self.dismiss_interruptions()
    ...
    async def get_pruned_interactive_tree(self, url: Optional[str] = None, headless: bool = True) -> str:
        """Extract a pruned interactable DOM list and inject visual markers."""
        if url:
            await self.get_accessibility_tree(url, headless=headless)
        else:
            await self._ensure_page(headless=headless)
        
        try:
            result = await self.page.evaluate(self.PRUNING_JS)
            self._pruned_elements = result
            
            # Format nicely for LLM
            lines = []
            for item in result:
                lines.append(f"[{item['index']}] {item['tag']} ({item['type']}): \"{item['text']}\"")
            
            url_str = self.page.url if self.page else "unknown"
            header = f"# Pruned Interactive Tree of {url_str}\n"
            header += f"# {len(result)} interactive elements\n\n"
            return header + "\n".join(lines)
        except Exception as e:
            return f"Error extracting pruned tree: {str(e)}"
    ...
    async def click(self, ref: str) -> str:
        """Click an element by @eN reference or pruned index."""
        if ref.isdigit():
            idx = int(ref)
            target = next((el for el in self._pruned_elements if el['index'] == idx), None)
            if not target:
                return f"Error: Pruned element [{idx}] not found."
            cx, cy = target['center']
            try:
                await self.page.mouse.click(cx, cy)
                return f"Clicked pruned element [{idx}] at ({cx:.0f}, {cy:.0f})"
            except Exception as e:
                return f"Error clicking [{idx}]: {str(e)}"
    ...
    async def fill(self, ref: str, text: str) -> str:
        """Fill text into an element with React prototype setter bypass or pruned index."""
        if ref.isdigit():
            idx = int(ref)
            target = next((el for el in self._pruned_elements if el['index'] == idx), None)
            if not target:
            ...
            try:
                ...
                return f"Filled pruned element [{idx}] with '{text}'"
            except Exception as e:
                return f"Error filling [{idx}]: {str(e)}"
        ...
        try:
            ...
            # Type text using human-like simulation
            import asyncio, random
            for char in text:
                await self.cdp.send("Input.dispatchKeyEvent", {"type": "keyDown", "text": char})
                await self.cdp.send("Input.dispatchKeyEvent", {"type": "keyUp", "text": char})
                delay = random.uniform(0.04, 0.15)
                if char in " .,@":
                    delay += 0.1
                await asyncio.sleep(delay)
    ...
    async def capture_screenshot(self) -> Optional[str]:
        """Capture screenshot of the current page with markers, returned as base64."""
        if not self.page:
            return None
        import base64
        try:
